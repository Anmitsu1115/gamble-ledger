...
import { Plus, ChevronLeft, ChevronRight, X, Trash2, Settings2, Pencil } from "lucide-react";

// ---------- Storage helpers (localStorage; this app runs as a standalone PWA) ----------
const STORAGE_KEY = "gamble-ledger:data-v2";
...
async function loadData() {
  try {
    ...
    // ignore, key doesn't exist or is invalid
  }
  return null;
}
...
// ---------- Main App ----------
export default function GambleLedger() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      await saveData({ categories, entries, history, activeCat });
      setSaveState("saved");
    }, 250);
    return () => clearTimeout(t);
  }, [categories, entries, history, activeCat, loaded]);
  ...
  return (
    <div style={styles.app}>
      ...
      <div style={styles.saveIndicator}>{saveState === "saving" ? "保存中…" : "保存済み"}</div>
...
const styles = {
  ...
  settingsBtn: { background: "rgba(255,255,255,0.1)", border: "none", borderRadius: 8, padding: 8, color: "#f3efe7", display: "flex", alignItems: "center", cursor: "pointer" },
